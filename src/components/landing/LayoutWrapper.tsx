...
export default function SynchronizedLayout() {
  ...
  // Removed backgroundColors from the dependency array since it's a static import
  ...
  const typeAnimationSequence = useMemo(
    () =>
      specialties.flatMap((specialty, index) => [() => scrollTo(index), specialty, 2000]).concat([() => scrollTo(0)]),
